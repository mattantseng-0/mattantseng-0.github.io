...
Based on the following figure we can see that increasing the friction causes damping behavior in the pendulum. A larger coefficient of friction causes the pendulum to converge to the fixed point faster. 
...
<img src="../../images/project_images/negative_friction_pendulum/negative_friction_pendulum.gif" alt="Pendulum with negative friction" width="100%"/>
...
Next, we know that applying torque to a motor res motor causes rotation. The motor torque constant can be used to calculate the torque being applied to the pendulum as a function of current. 

$$
    \tau_{motor} = K_m*i
$$

While not explicitly used in this paper, this equation can be helpful in determining the magnitude of negative friction applied by the motor.
...
## Circuit Diagram and Real World Implementation
We can model a motor with an inductor and resistor in series. The following diagram shows a simplistic circuit diagram for attaching the motor to the negative resistor. 
...
With this circuit diagram, we can then construct our system. By tuning the length/mass of the pendulum and the value of the resistors, it is possible to change the rate of $\theta$ growth.

<img src="../../images/project_images/negative_friction_pendulum/real_world_pendulum.gif" alt="Pendulum with different damping behavior" width="100%"/>
